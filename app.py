...
# ================= LOGGED IN DASHBOARD =================
    ...
    if st.session_state.user_role == "Admin":
        ...
        if choice == "⚙️ Manage Customers":
            ...
            with st.form("service_form"):
                ...
                if st.form_submit_button("💾 Add Service") and s_name.strip():
                    ...
                    st.success("Added!")
                    st.rerun()
            df_serv = pd.read_sql_query("SELECT id as ID, category as Category, service_name as 'Service', price_rate as 'दर / Rate (₹)' FROM services WHERE is_active = 1", conn)
            ...
            df_orders = pd.read_sql_query('''
                SELECT 
                    ...
                    o.items_summary as 'Items (मात्रा व दर सहित)',
        ...
        with tab_order:
            ...
            if services_list:
                s_dict = {f"[{s[1]}] {s[2]} (दर: ₹{s[3]})": s for s in services_list}
                sel_label = st.selectbox("🔍 आइटम चुनें:", list(s_dict.keys()))
                ...
                with col_r:
                    rate = st.number_input("💵 दर / Rate (₹):", value=float(s_item[3]), step=1.0)
                
                if st.button("➕ कार्ट में जोड़ें"):
                    item_total = qty * rate
                    st.session_state.cart.append({
                        "id": s_item[0], "name": s_item[2], "rate": rate, "qty": qty, "total": item_total
                    })
                    st.success("✅ कार्ट में जुड़ गया!")

        with tab_cart:
            st.subheader("🛍️ आपकी कार्ट (मात्र और दर अलग-अलग)")
            if st.session_state.cart:
                df_cart = pd.DataFrame(st.session_state.cart)
                df_cart.columns = ['ID', 'आइटम का नाम', 'दर (₹)', 'मात्रा (Qty)', 'कुल मूल्य (₹)']
                st.dataframe(df_cart[['आइटम का नाम', 'दर (₹)', 'मात्रा (Qty)', 'कुल मूल्य (₹)']], use_container_width=True)
                
                grand_total = df_cart['कुल मूल्य (₹)'].sum()
                st.markdown(f"### 💵 कुल योग: **₹{grand_total:,.2f}**")
                
                del_addr = st.text_area("🏠 डिलिवरी पता:", value=c_addr)
                if st.button("🚀 आर्डर फाइनल करें"):
                    summary_str = ", ".join([f"{i['name']} (दर: ₹{i['rate']} × मात्रा: {i['qty']} = ₹{i['total']})" for i in st.session_state.cart])
